...
class _AnotherScreenState extends State<AnotherScreen> {
  ...
  try {
    var output = await Tflite.runModelOnImage(
      path: image.path,
      numResults:
          54, //the amout of categories our neural network can predict (here no. of animals)
      threshold: 0.5,
      imageMean: 127.5,
      imageStd: 127.5,
    );
  ...
  loadModel() async {
    // Load all models at the start of your application
    ...
    print("Model Pne loaded successfully");
...
FutureBuilder<List<String>> (
  future: () async {
    try {
      // Run the main model
      var mainOutput = await Tflite.runModelOnImage(
        path: path_to_your_image, 
        //model: 'MainModel.tflite',
        numResults: 2,
        threshold:0.2,
        imageMean: 127.5,
        imageStd: 127.5,
      );
      print("Main model run successfully");
      print(mainOutput);

      // Decide which model to run based on the main model's output
      String model;
      if (mainOutput?[0]['label'] == 'Asthma') {
        model = 'NvsA.tflite';
      } else if (mainOutput?[0]['label'] == 'Bronchitis') {
        model = 'NvsB.tflite';
      } else {
        model = 'NvsC.tflite';
      }
      ...
      print("Selected model run successfully");
      print(output);
          ...
          child: Column(
            ...
            children: [
              Container(
                child: Center(
                  child: _loading == true
                      ? null //shows nothing if no picture selected
                      : Container(
                          child: Column(
                            children: [
                              SizedBox(
